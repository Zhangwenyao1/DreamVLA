...
from ..utils.visualize_utils import visualize_optical_flow
...
def patchify_depth(depths, patch_size):
    """
    imgs: (N, 1, H, W)
    x: (N, L, patch_size**2 *3)
    """

    assert depths.shape[2] == depths.shape[3] and depths.shape[2] % patch_size == 0

    h = w = depths.shape[2] // patch_size
    x = depths.reshape(shape=(depths.shape[0], 1, h, patch_size, w, patch_size))
    x = torch.einsum('nchpwq->nhwpqc', x)
    x = x.reshape(shape=(depths.shape[0], h * w, patch_size**2 * 1))
...
def train_one_epoch_calvin(
    ...
    for num_steps, batch_calvin in t:
        ...
        with autocast():  # image_primary, image_wrist, state, language_instruction
            arm_pred_action, gripper_pred_action, image_pred, arm_pred_state, gripper_pred_state, loss_arm_action, depth_pred, traj_pred = model(
        ...
        if args.loss_image and args.obs_pred:
            ...
            pred_image_example_primary = unpatchify(image_pred[:, 0, :, :, :])[0][0].permute(1, 2, 0).detach().cpu().numpy()
            ...
            label_image_example_primary = unpatchify(label_image_primary)[0][0].permute(1, 2, 0).detach().cpu().numpy()
            ...
            pred_image_example_secondary = unpatchify(image_pred[:, 1, :, :, :])[0][0].permute(1, 2, 0).detach().cpu().numpy()
            ...
            label_image_wrist_example = unpatchify(label_image_wrist)[0][0].permute(1, 2, 0).detach().cpu().numpy()
        ...
        if args.loss_depth and args.depth_pred:
            ...
            # 如果需要，可以使用 squeeze 去除单一维度（视具体 loss 计算需求而定）
            ...
            if args.use_dpt_head:
                ...
                label_depths_primary = depths_primary[:, args.future_steps:args.future_steps+args.sequence_length-args.atten_goal+args.pred_num-1, :].flatten(0,1)
                # label_image_wrist = images_wrist[:, args.future_steps:args.future_steps+args.sequence_length-args.atten_goal, :].flatten(0, 1)
                label_depths_wrist = depths_wrist[:, args.future_steps:args.future_steps+args.sequence_length-args.atten_goal+args.pred_num-1, :].flatten(0,1)
                label_depths_primary = patchify_depth(label_depths_primary, patch_size=args.patch_size)
                label_depths_wrist = patchify_depth(label_depths_wrist, patch_size=args.patch_size)
                
                label_depths_primary = label_depths_primary.view(bs, args.sequence_length-args.atten_goal+args.pred_num-1, *label_depths_primary.shape[1:])
                label_depths_primary = label_depths_primary.unfold(1, args.pred_num, 1).permute(0, 1, 4, 2, 3).flatten(0, 1)
                
                
                label_depths_wrist = label_depths_wrist.view(bs, args.sequence_length-args.atten_goal+args.pred_num-1, *label_depths_wrist.shape[1:])
                label_depths_wrist = label_depths_wrist.unfold(1, args.pred_num, 1).permute(0, 1, 4, 2, 3).flatten(0, 1)

                
                if args.flow_as_mask and 'tracks_l' in track_infos.keys():
                    loss_pred_depth = 0.5 * (torch.nn.functional.mse_loss(
                                    depth_x_pred*mask_reshape.to(depth_x_pred.device), 
                                    (label_depths_primary*(mask_reshape.to(depth_x_pred.device))).detach()) + 
                                    torch.nn.functional.mse_loss(
                                    depth_gripper_pred*wrist_masks_reshape.to(depth_gripper_pred.device), 
                                    (label_depths_wrist*wrist_masks_reshape.to(depth_gripper_pred.device)).detach()))
            ...
            pred_depth_example_primary = unpatchify(depth_x_pred)[0][0].permute(1, 2, 0).detach().cpu().numpy()
            ...
            label_depth_example_primary = unpatchify(label_depths_primary)[0][0].permute(1, 2, 0).detach().cpu().numpy()
            ...
            pred_depth_example_secondary = unpatchify(depth_gripper_pred)[0][0].permute(1, 2, 0).detach().cpu().numpy()
            ...
            label_depths_wrist_example = unpatchify(label_depths_wrist)[0][0].permute(1, 2, 0).detach().cpu().numpy()
            label_depths_wrist_example = (label_depths_wrist_example - label_depths_wrist_example.min()) / (label_depths_wrist_example.max() - label_depths_wrist_example.min())

            # before dynamic flow
            # pred_depth_example_primary = depth_x_pred[0][0].permute(1, 2, 0).detach().cpu().numpy()
            # pred_depth_example_primary = (pred_depth_example_primary - pred_depth_example_primary.min()) / (pred_depth_example_primary.max() - pred_depth_example_primary.min())
            # # pred_image_example_primary = (pred_image_example_primary * 255).astype(np.uint8)
            
            # label_depth_example_primary = label_depths_primary[0][0].permute(1, 2, 0).detach().cpu().numpy()
            # label_depth_example_primary = (label_depth_example_primary - label_depth_example_primary.min()) / (label_depth_example_primary.max() - label_depth_example_primary.min())
            # # pred_image_example_primary = (label_image_example_primary * 255).astype(np.uint8)
            
            
            # pred_depth_example_secondary = depth_gripper_pred[0][0].permute(1, 2, 0).detach().cpu().numpy()
            # pred_depth_example_secondary = (pred_depth_example_secondary - pred_depth_example_secondary.min()) / (pred_depth_example_secondary.max() - pred_depth_example_secondary.min())
            # # pred_image_example_secondary = (pred_image_example_secondary * 255).astype(np.uint8)
            
            # label_depth_wrist_example = label_depths_wrist[0][0].permute(1, 2, 0).detach().cpu().numpy()
        ...
        loss_calvin = args.loss_arm_action_ratio * loss_arm_action + args.loss_gripper_action_ratio * loss_gripper_action + 0.1 * loss_image + 0.001*loss_pred_depth + 0.1 * loss_pred_trajectory
        ...
        t.set_postfix({"avg loss": sum(mv_avg_loss[-avg_horizon:]) / avg_horizon, "loss": loss_calvin.item(), "loss_image": loss_image.item(), "loss_depth":loss_depth.item(), "loss_arm_action": loss_arm_action.item(), "loss_gripper_action": loss_gripper_action.item(), "loss_pred_trajectory": loss_pred_trajectory.item()})
...
def unpatchify(patches, patch_size=16, img_size=[224, 224]):
    """
    patches: [B, S, P, 196, 768] -> 还原到 [B, S, P, C, H, W]
    patch_size: 每个 patch 的大小，例如 16
    img_size: (H, W)，例如 (224, 224)
    """
    B, P, num_patches, patch_dim = patches.shape
    H, W = img_size
    grid_size = int(num_patches ** 0.5)  # 14 if num_patches=196

    # 恢复 patch 形状为 (patch_size, patch_size, C)
    C = patch_dim // (patch_size * patch_size)  # 假设默认 RGB 3通道
    patches = patches.view(B, P, grid_size, grid_size, patch_size, patch_size, C)

    # 调整维度顺序并合并
    img = patches.permute(0, 1, 6, 2, 4, 3, 5).contiguous()
    img = img.view(B, P, C, H, W)  # 还原为完整的 (B, S, P, C, H, W)
