...
class DiskCalvinDataset(BaseCalvinDataset):
    ...
    def _load_episode(self, idx: int, window_size: int) -> Dict[str, np.ndarray]:
        ...
        # if time.time()-start_time > 0.3:
            # print("sam_time", time.time()-start_time)
        # lhs: cotracker label
        # start_time = time.time()
        ...
        # if time.time()-start_time > 0.1:
            # print("traj_time", time.time()-start_time)
        return episode
    ...
    def _get_sequences(self, idx: int, window_size: int, head: bool=False) -> Dict:
        ...
        for step_id in range(start_id, end_id):
            ...
            if self.load_dino_features:
                ...
                data_dict["dino_static"] = dino_data[start_id:end_id]
                data_dict["dino_gripper"] = dino_data[start_id+seq_length//2:end_id+seq_length//2]
            if self.load_sam_features:
                seq_length = sam_data.shape[0]
                data_dict["sam_static"] = sam_data[start_id:end_id]
                data_dict["sam_gripper"] = sam_data[start_id+seq_length//2:end_id+seq_length//2]
            if self.load_track_labels:
                data_dict["traj_static"] = None
                data_dict["traj_gripper"] = None
                data_dict["visibility_static"] = None
                data_dict["visibility_gripper"] = None
